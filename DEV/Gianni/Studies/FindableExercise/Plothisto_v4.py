...
hFindSpeciesAll = fileFindSpeciesAll["findable-study/h2dPtVsCentrality_All_Findable"]
hFoundSpeciesAll = fileFindSpeciesAll["findable-study/h2dPtVsCentrality_All_Found"]
hGenSpecies = fileGen["strangederivedbuilder/h2dGenLambda"]
...
def plotEff(cent_bins, hNumerator, hDenominator, plot_name, ylabel):
    # Plot the histogram
    ...
    ## Rebinning
    if len(hDenoEdges_Centrality)!=len(hNumEdges_Centrality):
        print("------------------------------------------")
    ...
    for cent_bin in cent_bins:
        ...
        # Plot
        # print("Numerator_errors[{}-{}]".format(HistoEdges_Centrality[cent_bin], HistoEdges_Centrality[cent_bin+1]), Numerator_errors[cent_bin])
        # print("Denominator_errors[{}-{}]".format(HistoEdges_Centrality[cent_bin], HistoEdges_Centrality[cent_bin+1]), Denominator_errors[cent_bin])
        # print("Efficiency_error", Efficiency_error)
        # print("Numerator[cent_bin]", Numerator[cent_bin])
        # print("Denominator[cent_bin]", Denominator[cent_bin])
        # print("Efficiency[cent_bin]", Efficiency)
        ax.errorbar(Point_positions, Efficiency, yerr=Efficiency_error, xerr=bin_sizes, fmt='o', label='{}-{}%'.format(HistoEdges_Centrality[cent_bin], HistoEdges_Centrality[cent_bin+1]), markersize=4)

    #------------------------------------------------
    # Plot configs: 
    plt.figtext(0.275, 0.670, r"$\bf{ALICE MC}$" + '\n Pb-Pb at $\sqrt{s}=5.36$ TeV \n $|\mathrm{y}|<0.5$', ha='center') # ALICE
    ...
    plt.savefig('Results/{}.png'.format(plot_name), bbox_inches='tight')    
